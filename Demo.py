...
- Separate sections with clear headings"""
        ...
        Settings.llm = Cohere(
            model="large",  # Assuming this model exists in Cohere
            temperature=0.1,
            system_prompt=system_prompt,
        )

        index = VectorStoreIndex.from_documents(
            docs, node_parser=node_parser, show_progress=True
        )
        return index
...
with st.sidebar:
    ...
    if st.session_state.references:
        for doc_name, page in st.session_state.references:
            st.markdown(
                f"- [Source: {doc_name}, Page {page}](./data/{doc_name}.pdf#page={page})",
